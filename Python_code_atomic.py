...
Lambda0 = np.zeros( (numN, numL, numJ, numK, numF, numF, numN, numL, numJ, numK, numF, numF), dtype = np.complex)

L0 = np.zeros( (numN, numL, numJ, numK, numF, numF, numN, numL, numJ, numK, numF, numF), dtype = np.complex)

L2 = np.zeros( (numN, numL, numJ, numK, numF, numF, numN, numL, numJ, numK, numF, numF), dtype = np.complex)
'''
...
def rad_field_tensor(K, n0, n1, l0, l1, J0, J1, T, pert_index):
	...
	freq = energy(n0, l0, J0) - energy(n1, l1, J1)
	freq = np.abs(freq)/h
	...
	# We want to determine which portion of the code we want to locate
	...
	if K==0 and pert_index == False:
		...
		rad_field = weight*x*phase_deriv*pert_K0
...
def T_S(n0, n1, l0, l1, J0, J1, K0, K1, F0, F1, F2, F3, pert_index):
...
'''
We want to write code where we order values into Lambda, L0, or L2.
'''
					...
					for j2 in range(len(J2)):
						...
						for k1 in range(numK):
							for k2 in range(numK):
								...
								for f1 in range(len(F0)):
									for f2 in range( len(F1)):
										for f3 in range(len(F2)):
											for f4 in range(len(F3)):

												# Nhat term

												Nhat_term = -2*np.pi*complex(0,1)*Nhat(n1, n2, l1, l2, J1[j1], J2[j2], k1, k2, F1[f1], F2[f2], F3[f3], F4[f4])


												# Both the preturbed and unpreturbed T_A terms.
												
												TA_term_unpert = T_A(n1, n2, l1, l2, J1[j1], J2[j2], k1, k2, 0, F1[f1], F2[f2], F3[f3], F4[f4], False)
												TA_term_pert_K0 = T_A(n1, n2, l1, l2, J1[j1], J2[j2], k1, k2, 0, F1[f1], F2[f2], F3[f3], F4[f4], True)

												TA_term_pert_K2 = T_A(n1, n2, l1, l2, J1[j1], J2[j2], k1, k2, 2, F1[f1], F2[f2], F3[f3], F4[f4], True)



												# T_E term
									
												TE_term = T_E(n1, n2, l1, l2, J1[j1], J2[j2], k1, k2, F1[f1], F2[f2], F3[f3], F4[f4])

												# Both the preturbed and unpreturbed T_S terms.	
		
												TS_term_unpert = T_A(n1, n2, l1, l2, J1[j1], J2[j2], k1, k2, 0, F1[f1], F2[f2], F3[f3], F4[f4], False)
												TS_term_pert_K0 = T_A(n1, n2, l1, l2, J1[j1], J2[j2], k1, k2, 0, F1[f1], F2[f2], F3[f3], F4[f4], True)

												TS_term_pert_K2 = T_A(n1, n2, l1, l2, J1[j1], J2[j2], k1, k2, 2, F1[f1], F2[f2], F3[f3], F4[f4], True)

												# Both the preturbed and unpreturbed R_A terms.

												RA_term_unpert = R_A(n1, l1, J1[j1], I, k1, k2, F1[f1], F2[f2], F3[f3], F4[f4], False)
												RA_term_pert = R_A(n1, l1, J1[j1], I, k1, k2, F1[f1], F2[f2], F3[f3], F4[f4], True)

												# R_E term

												RE_term = R_E(n1, l1, J1[j1], I, k1, k2, F1[f1], F2[f2], F3[f3], F4[f4])

												# Both the preturbed and unpreturbed R_S terms.
												
												RS_term_unpert = R_S(n1, l1, J1[j1], I, k1, k2, F1[f1], F2[f2], F3[f3], F4[f4], False)

												RS_term_pert = R_S(n1, l1, J1[j1], I, k1, k2, F1[f1], F2[f2], F3[f3], F4[f4], True)

												# Organize terms into respective matrices. Focus on Lambda0 first.

												

												Lambda0[n1,l1, j1, k1, f1, f2, n2, l2, j2, k2, f3, f4] += TE_term
												Lambda0[n1,l1, j1, k1, f1, f2, n2, l2, j2, k2, f3, f4] += TA_term_unpert
												Lambda0[n1,l1, j1, k1, f1, f2, n2, l2, j2, k2, f3, f4] += TS_term_unpert

												if n1==n2 and l1==l2 and j1==j2:

													Lambda0[n1,l1, j1, k1, f1, f2, n2, l2, j2, k2, f3, f4] += RE_term
													Lambda0[n1, l1, j1, k1, f1, f2, n2, l2, j2, k2, f3, f4] += RA_term_unpert
													Lambda0[n1,l1, j1, k1, f1, f2, n2, l2, j2, k2, f3, f4] += RS_term_unpert
				

												# Focus on L0 term now.

												L0[n1, l1, j1, k1, f1, f2, n2, l2, j2, k2, f3, f4] += TA_term_pert + TS_term_pert




												L0[n1,l1,j1,k1,f1, f2, n2, l2, j2, k2, f3, f4] += TA_term_pert + TS_term_pert + RA_term_pert + RS_term_pert
