...
def energy(n, l, J, I, F):
	...
	energy_0th = -13.6*eV_to_Joules / n**2
...
def rad_field_tensor(K, n0, n1, l0, l1, freq, T):
	...
	if K == 0:
		...
		rad_field = (1/np.sqrt(2)) * weight * BB_deriv * pert_K2
	else:
		rad_field = 0
...
def T_A(n0, n1, l0, l1, J0, J1, K0, K1, F0, F1, F2, F3, freq):
	...
	for Kr in range(3):
		...
		temp *= rad_field_tensor(Kr, n0, n1, l0, l1, freq)
	...
	for n_level in range(Nmax+1):
		for l_level in range(Nmax):
			...
			for j_index in range(2):
				...
				if energy(n_level, l_level, J_u) > energy(n, l, J):
		...
		for n_level in range(Nmax+1):
			for l_level in range(Nmax):
				...
				for j_index in range(2):
					...
					if energy(n_level, l_level, J_l) < energy(n, l, J):
