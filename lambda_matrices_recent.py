...
# Creating a list of perturbation variables

atomic_var = [N0,L0,J0,K0,F0,F1,N1,L1,J1,K1,F2,F3]
tk_var = [wave_num, delta_g, delta_b, phi, psi, h_scalar, h_prime, eta_scalar, eta_prime, t_g, t_b, shear_g, pol_0]
thermo_var = [redshift, T, x_e, H_param, optical_depth, num_baryon, num_H_tot]

# Radiation field function

def rad_field_tensor_BB(Kr, atomic_var, tk_var, thermo_var):
    ...
    # Lya photon frequency
    
    freq_lya = energy_noF(N, L, J) - energy_noF(N_l, L_l, J_l)
    freq_lya = np.abs(freq_lya)/h
    ...
    if (N0==1 and N1==2 and L0==0 and L1==1 and J0==0.5) or (N0==2 and N1==1 and L0==1 and L1==0 and J1==0.5):
        
        if K0 == 0 and K1 == 0  and Kr == 0 and pert_index == False:
...
def Lambda_lymann_lines_BB(Kr, atomic_var, tk_var, thermo_var, pert_index):
    ...
    if N0 == 2 and L0 == 1 and N1 == 1 and L1 == 0 and J1 == 0.5:
        ...
        if Kr==0 and K0==K1 and F2==F3 and pert_index==False:
            
            term = B_Einstein_abs
            term *= 2*np.sqrt(3)*np.sqrt( (2*F0+1)*(2*F1+1)*(2*K0+1) )
            term *= np.float( wigner_9j(F0, F2, 1, F1, F3, 1, K0, K0, 1) )
            term *= np.float( wigner_6j(J0,0.5,1,F2,F0,0.5)*wigner_6j(J0,0.5,1,F2,F3,0.5) )
            
            term *= rad_field_tensor_BB(0,atomic_Var, tk_var,thermo_var, False)
            
        elif Kr==0 and K0==0 and K1==0 and F2==F3 and pert_index==True:
            
            term = B_Einstein_abs
            term *= 2*np.sqrt((2*F2+1)*(2*F0+1) )
            term *= np.abs( wigner_6j(J0, 0.5, 1, F2, F0, 0.5) )**2
            
            term *= rad_field_tensor_BB(0,atomic_var, tk_var,thermo_var, True)
            
        elif Kr==2 and K0==2 and K1==0 and F2==F3 and pert_index==True:
            
            term = B_Einstein_abs
            term *= 2*np.sqrt(15)*np.sqrt( (2*F0+1)*(2*F1+1)*(2*F2+1)*(2*F3+1) )
            term *= np.float( wigner_9j(F0,F2,1,F1,F2,1,2,0,2) )
            term *= np.float( wigner_6j(1.5,0.5,1,F2,F0,0.5)*wigner_6j(1.5,0.5,1,F2,F1,0.5) )
            
            term *= rad_field_tensor_BB(2,atomic_var, tk_var,thermo_var, True)
            
        else:
            
            term = 0
    # The T_E (transmission spontaneous emission terms) are listed below:
        
    if N0==1 and L0==0 and J0==0.5 and N1==2 and L1==1:
        if Kr==0 and K0==K1 and F0==F1 and F2==F3 and pert_index==False:
                
                term = A_Einstein
                term *= (2*J1+1)*np.sqrt( (2*F0+1)*(2*F2+1) )
                term *= np.abs( wigner_6j(J1,0.5,1,F0,F2,0.5) )**2
                
        
            
    # All the R_E (relaxation spontaneous emission terms) are listed below:
            
    if N0==2 and L0==1 and N1==2 and L1==1:        
    ...
    if N0==1 and L0==0 and N1==1 and L1==0:       
        ...
        if Kr==0 and K0==K1 and F0==F2 and F1==F3 and pert_index==True:
...
def Lambda_lymann_lines_source(Kr, atomic_var, tk_var, thermo_var, rho_1s, pert_index):
    ...
    # Calculating the total population in the 1s state
    
    X_1s_tot = rho_1s[0,0]+ np.sqrt(3)*rho_1s[1,1]
	...
	for F_l in range(2):
		for F_l_prime in range(2):
		
			dummy_term = np.sqrt( (2*F_l+1)*(2*F_l_prime+1) )
			dummy_term *= np.float( wigner_9j(F0,F_l,1,F1,F_l_prime,1,K0,K1,Kr) )
	...
	if Kr==0 and F0==F1 and F2==F3 and pert_index == False:
		...
		term = -P_esc*weight*B_Einstein_abs*Hubble_pert(0,h_scalar,h_prime,eta_scalar,t_b)/3
		term *= 2*np.sqrt( 15*(2*F0+1)*(2*F1+1)*(2*F2+1) )
...
def line_strength_sum(F, Fprime, K, K_l, K_r, X_1s):
    
    term = 0
    
    for F_l in range(2):
        
        term = np.sqrt(2*F_l+1)
        term *= np.float(wigner_9j(F,F_l,1,Fprime,F_l,1,K,K_l,K_r) )
        term *= np.float( wigner_6j(J,0.5,1,F_l,F,0.5)*wigner_6j(J,0.5,1,F_l,Fprime,0.5) )
        term *= X_1s[F_l]/(X_1s[0]+X_1s[1])
        
    return term



def rad_field_tensor_lya_BB(N0,N1,L0,L1,J0,J1,K0,K1,Kr,F0,F1,F2,F3,T,h_scalar,h_prime,eta_scalar,t_b,optical_depth,pert_index):
 
    P_esc = 1/optical_depth # escape probability
...
def Lambda_Lymann_lines(N0,N1,L0,L1,J0,J1,K0,K1,Kr,F0,F1,F2,F3,T,h_scalar,h_prime,eta_scalar,t_b,optical_Depth_pert_index):
    ...
    if N0==2 and L0==1 and N0==1 and L0==0:
        ...
        B_Einstein_stim = 32*np.pi**4/(3*h**2*c**2)
        B_Einstein_stim *= np.abs( dipole_element(N,N_l,L,L_l,J,J_l))**2
        A_Einstein = weight*B_Einstein_stim
        
        # Below are the Black Body T_A terms
        
        if Kr == 0 and K == K_l and F_l == F_l_prime and pert_index == False:
            
            term = 2*np.sqrt(3)*(2*F_l+1)
            term *= np.sqrt((2*F+1)*(2*Fprime+1)*(2*K+1))
            term *= np.float(wigner_9j(F,Fl,1,Fprime,Fl,1,K,K,0))
            term *= np.float(wigner_6j(J,0.5,1,F_l,F,0.5)*wigner_6j(J,0.5,1,F_l,Fprime,0.5))
            term *= P_esc*weight*phase_space*B_Einstein_stim
            
        elif Kr == 0 and K == K_l and K==0 and F==Fprime and F_l == F_l_prime and pert_index == True:
            
            term = B_Einstein_stim
            term *= 2*np.sqrt((2*Fl+1)*(2*F+1))
            term *= np.float( wigner_6j(J,0.5,1,Fl,F,0.5) )**2
            
            term1 = P_esc*weight*phase_space*Hubble_pert(0, h_scalar, h_prime, eta_scalar, t_b)
            term2 = -2*P_esc*(kB*T)**3*x**4*phase_deriv*Theta_0/(h*c)**2
            
            term *= term1 + term2
            
        elif Kr == 2 and K == Kr and K_l == 0 and F_l == F_l_prime and pert_index == True:
            
            term = B_Einstein_stim
            term *= 2*np.sqrt(3)*np.sqrt( (2*F+1)*(2*Fprime+1)*(2*K+1) )
            term *= np.sqrt((2*F+1)*(2*Fprime+1)*(2*K+1))
            term *= np.float(wigner_9j(F,Fl,1,Fprime,Fl,1,2,0,2))
            term *= np.float(wigner_6j(J,0.5,1,F_l,F,0.5)*wigner_6j(J,0.5,1,F_l,Fprime,0.5))    
            
            term *= P_esc*np.sqrt(2)*(kB*T)**3*x**4*phase_deriv*Theta_2/(h*c)**2
            
        # Below are the source terms related to T_A
    ...
    if N0 == 2 and L0 == 1 and N1 == 2 and L1 == 1:
        ...
        freq = energy_noF(N,L,J) - energy_noF(N_u,L_u,J_u)
        freq = np.abs(freq)/h
        ...
        if Kr==0 and K == K_r and F_l == F_l_prime and pert_index == False:
            
            term = (1-P_esc)*weight*B_Einstein_stim
            term *= 2*np.sqrt(3)*np.sqrt( (2*F+1)*(2*Fprime+1)*(2*K+1)*(2*F_u+1) )
            term *= sum_1s
            
        elif Kr==0 and K == K_r and F_l == F_l_prime and pert_index == True:
        
            term = P_esc*weight*B_Einstein_stim
            term *= 2*np.sqrt(3)*np.sqrt( (2*F+1)*(2*Fprime+1)*(2*K+1)*(2*F_u+1) )
            term *= sum_1s
            
            term *= (delta_b/X_1s - Hubble_pert(0, h_scalar, h_prime, eta_scalar, t_b))

        elif Kr==2 and K_u==0 and K==K_r and F_l==F_l_prime and pert_index==True:
            
            for F_l in range(2):
                
                sum_1s = np.sqrt(2*F_l+1)
                sum_1s *= np.float( wigner_9j(F,F_l,1,Fprime,F_l,1,2,0,2) )
                sum_1s *= np.float(wigner_6j(1.5,0.5,1,F_l,F,0.5)*wigner_6j(1.5,0.5,1,F_l,Fprime,0.5) )
                sum_1s *= X_1s_F/X_1s

            term = - np.sqrt(2)*P_esc*weight/np.sqrt(15)
            tarm *= np.sqrt( (2*F+1)*(2*Fprime+1) )
            term *= sum_1s
            term *= np.sqrt(2*F_u+1)
            
            


            
        elif Kr==2 and K_u==2 and K==K_r and F_l==F_l_prime and pert_index == True:
            ...
            term = 14*np.sqrt(2)*(1-P_esc)*weight/np.sqrt(15)
            term *= B_Einstein_stim
            term *= np.sqrt( (2*F+1)*(2*Fprime+1) )
            term *= (-1)**F_u_prime * np.sqrt( (2*F_u+1)*(2*F_u_prime+1))
            term *= np.float( wigner_6j(1.5,1.5,2,F_u_prime,F_u,1) )
...
def Lambda_Lymann_lines(N0,N1,L0,L1,J0,J1,K0,K1,K_r,F0,F1,F2,F3,T,h_scalar,h_prime,eta_scalar,t_b, optical_depth,pert_index):
    ...
    if N0==2 and L0==1 and N1==1 and L1==0:
        ...
        if K==2 and K_l==0 and Kr==0 and J
